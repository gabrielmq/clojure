...
(def transacoes-aleatorias
  '({:valor 7.0M :tipo "despesa" :rotulos ["sorvete" "entreternimento"]}
...
(against-background
  [(before :facts [(iniciar-servidor porta-padrao) (db/limpar)])
  ...
  (against-background
    ...
    (fact "Existe 1 receita com rótulo 'salário'"
          (count (:transacoes (json/parse-string (conteudo "/transacoes?rotulos=salário") true)))
          => 1)

    (fact "Existem 2 despesas com rótulo 'livro' ou 'curso'"
          (count (:transacoes (json/parse-string (conteudo "/transacoes?rotulos=livro&rotulos=curso") true)))
          => 2)

    (fact "Existem 2 despesas com rótulo 'educação'"
          (count (:transacoes (json/parse-string (conteudo "/transacoes?rotulos=educação") true)))
          => 2)))